...
//  approval, mode, plan, cwd, sessionId, dirs} 写入，操作（207 set）后插件写 type=1
...
///  mode:{current,options[{value,label}]}, plan:{on}, cwd, sessionId, dirs[]}。
/// preset/approval/mode 可能整体缺失（旧版插件）或 options 为空数组（部署未提供 preset 服务）；
/// 未设置/非法时返回 nil。
