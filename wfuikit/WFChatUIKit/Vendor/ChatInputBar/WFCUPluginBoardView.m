...
- (NSMutableArray *)pluginItems {
    if (!_pluginItems) {
        if (self.hasVoip) {
            _pluginItems = [@[
                              [[PluginItem alloc] initWithTitle:@"相册" image:[UIImage wf_imageNamed:@"chat_input_plugin_album"] tag:1],
                              [[PluginItem alloc] initWithTitle:@"拍摄" image:[UIImage wf_imageNamed:@"chat_input_plugin_camera"] tag:2],
                              [[PluginItem alloc] initWithTitle:@"位置" image:[UIImage wf_imageNamed:@"chat_input_plugin_location"] tag:3],
#if WFCU_SUPPORT_VOIP
                              [[PluginItem alloc] initWithTitle:@"视频通话" image:[UIImage wf_imageNamed:@"chat_input_plugin_video_call"] tag:4],
#endif
                              [[PluginItem alloc] initWithTitle:@"文件" image:[UIImage wf_imageNamed:@"chat_input_plugin_file"] tag:5]
                              ] mutableCopy];
        } else {
            _pluginItems = [@[
                              [[PluginItem alloc] initWithTitle:@"相册" image:[UIImage wf_imageNamed:@"chat_input_plugin_album"] tag:1],
