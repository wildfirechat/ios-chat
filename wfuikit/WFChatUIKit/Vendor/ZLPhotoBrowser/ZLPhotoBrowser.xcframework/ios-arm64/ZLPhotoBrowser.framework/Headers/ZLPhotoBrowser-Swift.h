...
/// \param completion Call this closure when you want the camera to proceed with capture.
///
/// \param isCapturing Boolean indicating if a capture operation is already in progress
///
@property (nonatomic, copy) void (^ _Nullable willCaptureBlock)(void (^ _Nonnull)(void), BOOL);
...
SWIFT_CLASS("_TtC14ZLPhotoBrowser20ZLImageNavController")
@interface ZLImageNavController : UINavigationController
...
- (ZLPhotoPreviewSheet * _Nonnull)showPreviewWithAnimate:(BOOL)animate sender:(UIViewController * _Nonnull)sender;
/// warning:
/// When calling this method in OC language, make sure that the <code>sender</code> is not zero
- (ZLImageNavController * _Nonnull)showPhotoLibraryWithSender:(UIViewController * _Nonnull)sender;
/// 传入已选择的assets，并预览
...
@interface ZLPhotoPicker (SWIFT_EXTENSION(ZLPhotoBrowser))
/// 传入已选择的assets，并预览
- (ZLImageNavController * _Nonnull)previewAssetsForSwiftUIWithAssets:(NSArray<PHAsset *> * _Nonnull)assets index:(NSInteger)index isOriginal:(BOOL)isOriginal showBottomViewAndSelectBtn:(BOOL)showBottomViewAndSelectBtn SWIFT_WARN_UNUSED_RESULT;
@end


SWIFT_CLASS("_TtC14ZLPhotoBrowser19ZLPhotoPreviewSheet") SWIFT_DEPRECATED_MSG("Please use ZLPhotoPicker instead. The permission of ZLPhotoPreviewSheet will be changed to private later.")
...
///
- (nonnull instancetype)initWithResults:(NSArray<ZLResultModel *> * _Nullable)results;
- (nonnull instancetype)initWithModels:(NSArray<ZLPhotoModel *> * _Nullable)models;
...
@property (nonatomic, strong) UIColor * _Nonnull noLibraryAuthTitleAndDescColor;
/// When you don’t have access to the album, go to the system settings button title color.
/// 无相册访问权限时，前往系统设置按钮标题颜色
@property (nonatomic, strong) UIColor * _Nonnull noLibraryAuthGotoSettingBtnTitleColor;
...
- (ZLPhotoUIConfiguration * _Nonnull)noLibraryAuthTitleAndDescColor:(UIColor * _Nonnull)color;
- (ZLPhotoUIConfiguration * _Nonnull)noLibraryAuthGotoSettingBtnTitleColor:(UIColor * _Nonnull)color;
