...
/**
 ...
 @param userId 用户id
 */
- (void)didReceiveData:(NSData *_Nonnull)data isBinary:(BOOL)isBinary fromUser:(NSString *_Nonnull)userId;
...
/**
 ...
 @param conversation 通话所在会话
 @param callExtra 通话附加信息
 @param pstnType PSTN类型，0普通音视频，1仅音频
 @param pstnNumber PSTN号码
 @param sessionDelegate 通话Session的监听
 @return 通话Session
 */
- (WFAVCallSession *)startCall:(NSArray<NSString *> *)targetIds
                  ...
                  conversation:(WFCCConversation *)conversation
                      pstnType:(int)pstnType
                    pstnNumber:(NSString *)pstnNumber
...
/**
 发送数据，所以已连接的通话或会议成员都能收到。
 
 @param data 数据
 @param isBinary 是否二进制

 @return TRUE 发送成，FALSE发送失败。
 */
- (BOOL)sendData:(NSData *_Nonnull)data isBinary:(BOOL)isBinary;
@end
